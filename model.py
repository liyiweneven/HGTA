...
class Model(nn.Module):
    def __init__(self, config: Config):
        ...
        elif config.news_encoder == 'CAUM':
            self.news_encoder = newsEncoders.CAUM(config)
        elif config.news_encoder == 'SCA':
            self.news_encoder = newsEncoders.SCA(config)
        elif config.news_encoder == 'SCA_wo_BiLSTM':
            self.news_encoder = variantEncoders.SCA_wo_BiLSTM(config)
        else:
        ...
        elif config.user_encoder == 'MULTI':
            self.user_encoder = userEncoders.MULTI(self.news_encoder, config)
        elif config.user_encoder == 'Candi':
            self.user_encoder = userEncoders.Candi(self.news_encoder, config)
        elif config.user_encoder == 'SCA_wo_Multi':
            self.user_encoder = variantEncoders.SCA_wo_Multi(self.news_encoder, config)
        elif config.user_encoder == 'SCA_wo_Threshold':
            self.user_encoder = variantEncoders.SCA_wo_Threshold(self.news_encoder, config)
        else:
    ...
    def forward(self, user_ID, user_category, user_subCategory, user_title_text, user_title_mask, user_title_entity,
                user_content_text, user_content_mask, user_content_entity, user_history_mask,news_category, news_subCategory, news_title_text, news_title_mask, news_title_entity, news_content_text,
        ...
        news_representation = self.news_encoder(news_title_text, news_title_mask, news_title_entity, news_content_text,
                                                news_content_mask, news_content_entity, news_category,
                                                news_subCategory,user_embedding)  # [batch_size, 1 + negative_sample_num, news_embedding_dim]
        user_representation = self.user_encoder(user_ID,user_title_text, user_title_mask, user_title_entity, user_content_text,
                                                user_content_mask, user_content_entity, user_category, user_subCategory,
                                                user_history_mask,user_embedding, news_representation)#user_ID ,
