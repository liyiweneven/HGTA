    ...
    def train(self):
        ...
        for e in tqdm(range(1, self.epoch + 1)):
            ...
            for (user_ID, user_category, user_subCategory, user_title_text, user_title_mask, user_title_entity,
                 ...
                 news_content_text, news_content_mask, news_content_entity) in train_dataloader:
                ...
                logits = model(user_ID, user_category, user_subCategory, user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask, user_content_entity, user_history_mask, \
        ...
        with open('output.txt', 'a') as file:
            file.write('Training : ' + model.model_name + ' #' + str(self.run_index) + ' completed\nDev criterions:')
            file.write('AUC : %.4f' % self.auc_results[self.best_dev_epoch - 1])
            file.write('MRR : %.4f' % self.mrr_results[self.best_dev_epoch - 1])
            file.write('nDCG@5 : %.4f' % self.ndcg5_results[self.best_dev_epoch - 1])
            file.write('nDCG@10 : %.4f' % self.ndcg10_results[self.best_dev_epoch - 1])
...
def distributed_train(rank, model: nn.Module, config: Config, mind_corpus: MIND_Corpus, run_index: int):
    ...
    for e in tqdm(range(1, epoch + 1)):
        ...
        model.train()
        epoch_loss = 0
        for (user_ID, user_category, user_subCategory, user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask, user_content_entity, user_history_mask,\
            news_category, news_subCategory, news_title_text, news_title_mask, news_title_entity, news_content_text, news_content_mask, news_content_entity) in train_dataloader:
            user_ID = user_ID.cuda(non_blocking=True)                                                                                                                       # [batch_size]
            user_category = user_category.cuda(non_blocking=True)                                                                                                           # [batch_size, max_history_num]
            user_subCategory = user_subCategory.cuda(non_blocking=True)                                                                                                     # [batch_size, max_history_num]
            ...
            optimizer.step()
        with open('output.txt', 'a') as file:
            file.write('rank %d : Epoch %d : train done' % (rank, e))
            file.write('rank %d : loss = %.6f' % (rank, epoch_loss / len(train_dataset) * world_size))
        ...
        if rank == 0:
            ...
            if dev_criterion == 'auc':
                ...
                if avg >= best_dev_avg:
                    ...
                    epoch_not_increase += 1
            with open('output.txt', 'a') as file:
                file.write('Best epoch :', best_dev_epoch)
                if dev_criterion == 'auc':
                    file.write('Best AUC : %.4f' % best_dev_auc)
                elif dev_criterion == 'mrr':
                    file.write('Best MRR : %.4f' % best_dev_mrr)
                elif dev_criterion == 'ndcg5':
                    file.write('Best nDCG@5 : %.4f' % best_dev_ndcg5)
                elif dev_criterion == 'ndcg10':
                    file.write('Best nDCG@10 : %.4f' % best_dev_ndcg10)
                else:
                    file.write('Best avg : ' + str(best_dev_avg))
    ...
    if rank == 0:
        ...
        with open('output.txt', 'a') as file:
            file.write('Training : ' + model_name + ' #' + str(run_index) + ' completed\nDev criterions:')
            file.write('AUC : %.4f' % auc_results[best_dev_epoch - 1])
            file.write('MRR : %.4f' % mrr_results[best_dev_epoch - 1])
            file.write('nDCG@5 : %.4f' % ndcg5_results[best_dev_epoch - 1])
            file.write('nDCG@10 : %.4f' % ndcg10_results[best_dev_epoch - 1])
