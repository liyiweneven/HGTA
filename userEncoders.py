...
from layers import MultiHeadAttention, Attention, ScaledDotProduct_CandidateAttention, CandidateAttention, GCN,Sparse_De_MHAtt,LayerNorm,FFN,Sparse_En_MHAtt,MultiHeadSelfAttention,SelfAttention
...
class LSTUR(UserEncoder):
    ...
    def forward(self, user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask, user_content_entity, user_category, user_subCategory, \
                user_history_mask, user_embedding, candidate_news_representation):
        batch_size = user_title_text.size(0)
        news_num = candidate_news_representation.size(1)
...
class MULTI(UserEncoder):
    def __init__(self, news_encoder: NewsEncoder, config: Config):
        super(MULTI, self).__init__(news_encoder, config)
        self.attention = Attention(self.news_embedding_dim, config.attention_dim)
        self.Sparse_De_MHAtt = Sparse_De_MHAtt(400, 20)
        self.Sparse_En_MHAtt = Sparse_En_MHAtt(400,20)
        self.layernorm = LayerNorm(400)
        self.multi_head_self_attention=MultiHeadSelfAttention(400,20)
        self.multi_self_attention = SelfAttention(400,8)
        self.affine1 = nn.Linear(in_features=400, out_features=200, bias=True)
        self.affine2 = nn.Linear(in_features=200, out_features=1, bias=False)
        self.dropout = nn.Dropout(p=0.1)
        self.lin = nn.Linear(800,400)
    ...
    def _scaled_dp_attention(self, user_ID,query, key, value):
        ...
        t = 0.01
        attn = self.softmax(scores/t)
        sparse_map = torch.full_like(scores, 0.019)  # 0.021 0.014
    ...
    def forward(self,user_ID,user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask,user_content_entity, user_category, user_subCategory,user_history_mask, user_embedding,candidate_news_representation):
        news_num = candidate_news_representation.size(1)
        history_embedding = self.news_encoder(user_title_text, user_title_mask, user_title_entity, \
                                              user_content_text, user_content_mask, user_content_entity, \
                                              user_category, user_subCategory,user_embedding)            # [batch_size, max_history_num, news_embedding_dim][16, 50, 400]

        '''user_reprs = self.Sparse_De_MHAtt(history_embedding, history_embedding, history_embedding)
        user_reprs = self.layernorm(user_reprs)'''

        #candidate_news_representation=self.Sparse_En_MHAtt(candidate_news_representation,candidate_news_representation,candidate_news_representation)

        user_reprs = self.multi_head_self_attention(user_ID,history_embedding,history_embedding)#原换成_scaled_dp_attention
        #user_reprs = self.Sparse_De_MHAtt(history_embedding, history_embedding, history_embedding)
        #user_reprs = self._scaled_dp_attention(user_ID, candidate_news_representation, user_reprs, user_reprs)
        #feature = torch.cat((user_reprs,candidate_news_representation),dim=-1)
        #feature = self.lin(feature)
        #user_representation = self.attention(user_reprs).unsqueeze(dim=1).expand(-1, news_num, -1)#[16, 400]
        user_representation = self.multi_self_attention(candidate_news_representation, user_reprs, user_reprs)
        return user_representation


class Candi(UserEncoder):
    def __init__(self, news_encoder: NewsEncoder, config: Config):
        super(Candi, self).__init__(news_encoder, config)
        self.attention = Attention(self.news_embedding_dim, config.attention_dim)
        self.multi_head_self_attention=MultiHeadSelfAttention(400,20).to(self.device)
        self.affine1 = nn.Linear(in_features=400, out_features=200, bias=True)
        self.affine2 = nn.Linear(in_features=200, out_features=1, bias=False)
        self.dropout = nn.Dropout(p=0.1)
        self.lin = nn.Linear(800,400)
        ...
        self.lin2=nn.Linear(800,400)
        self.lin3=nn.Linear(50,1)

        self.softmax = nn.Softmax(dim=-1)

        self.multihead_attention = nn.MultiheadAttention(400,20)
        self.linear1= nn.Linear(400 * 4, 400)
    def initialize(self):
        self.attention.initialize()
    ...
    def _scaled_dp_attention(self, query, key, value):
        ...
        attn_output = torch.matmul(attn_weights, value)
        return attn_output
    def forward(self, user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask,user_content_entity, user_category, user_subCategory,user_history_mask, user_embedding,candidate_news_representation):
        batch_size=candidate_news_representation.size(0)
        can_news_num = candidate_news_representation.size(1)
        history_embedding = self.news_encoder(user_title_text, user_title_mask, user_title_entity, \
                                              user_content_text, user_content_mask, user_content_entity, \
                                              user_category, user_subCategory,user_embedding)            # [batch_size, max_history_num, news_embedding_dim][16, 50, 400]
        his_news_num=history_embedding.size(1)

        #print(history_embedding.size())#[8,50,400]
        #print(candidate_news_representation.size())#[8, 5, 400]

        # -----------------------------------------------------利用CNN来捕获短期兴趣
        #his_cnn = self.CNN(history_embedding.permute(0,2,1)).permute(0,2,1)#[batch_size, max_history_num, news_embedding_dim]

        # candi-cnn

        his_cnn = history_embedding.unsqueeze(dim=1).repeat(1,can_news_num ,1,1)
        cdd_news_repr = candidate_news_representation.unsqueeze(dim=2).repeat(1,1,his_news_num ,1)
        ...
        his_cnn_repr = self.lin(his_cnn_repr)  #[8, 5, 50, 400]


        # -----------------------------------------------------利用自注意力来捕获长期兴趣
        his_repr=his_cnn_repr.view(-1,his_news_num,400)
        his_news_repr, _ = self.multihead_attention(his_repr,his_repr,his_repr)#[8, 5, 50, 20]
        his_news_repr=his_news_repr .view(batch_size,can_news_num,his_news_num,-1)
        his = torch.cat((clicked_news_cnn, his_news_repr), dim=-1)

        his=self.lin2(his)#[16, 5, 50, 400]
        #user_reprs=self.lin2(torch.cat((his,cdd_news_repr),dim=-1))
        #user_representation = self.dense_att(user_reprs.transpose(-1,-2)).squeeze(-1)

        #print(user_reprs.size())
        #his=self.lin3(his.transpose(3,1)).squeeze(dim=-1).transpose(-1,-2)

        user_reprs= self._scaled_dp_attention(cdd_news_repr,his,his)

        user_representation=self.lin3(user_reprs.transpose(-1,-2)).squeeze(dim=-1)
        #user_representation = self.attention(user_reprs).unsqueeze(dim=1).expand(-1,can_news_num, -1)

        return user_representation#[16, 5, 400]