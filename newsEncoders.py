...
from layers import Conv1D, Conv2D_Pool, MultiHeadAttention, Attention, ScaledDotProduct_CandidateAttention, CandidateAttention,decay_Attention
import numpy as np
...
class SCA(NewsEncoder):
    def __init__(self, config: Config):
        super(SCA, self).__init__(config)
        self.max_title_length = config.max_title_length
        self.max_content_length = config.max_abstract_length
        self.cnn_kernel_num = config.cnn_kernel_num
        self.news_embedding_dim = config.cnn_kernel_num
        ...
        self.affine1 = nn.Linear(config.cnn_kernel_num, config.attention_dim, bias=True)
        self.affine2 = nn.Linear(config.attention_dim, 1, bias=False)
        self.affine3 = nn.Linear(config.cnn_kernel_num*2, config.cnn_kernel_num, bias=False)

        self.ReLU = nn.ReLU()
        self.lstm_net = nn.LSTM(300, 200, num_layers=2, dropout=0.1, bidirectional=True)
        # nn.init.xavier_normal_(self.lstm_net)
        self.lstm_net2 = nn.LSTM(400, 200, num_layers=2, dropout=0.1, bidirectional=True)
        self.attention_layer = nn.Sequential(
            nn.Linear(200, 200),
            nn.ReLU(inplace=True)
        )
        self.lin= nn.Linear(200, 400, bias=False)
        self.lin1 = nn.Linear(800, 400, bias=False)
        self.lin2 = nn.Linear(128, 1, bias=False)
        self.lin3= nn.Linear(20, 1, bias=False)
        self.decay_attention = decay_Attention(config.cnn_kernel_num, 16)
    ...
    def forward(self,title_text, title_mask, title_entity, content_text, content_mask, content_entity, category, subCategory,user_embedding):
        ...
        content_w = self.dropout(self.word_embedding(content_text)).view([batch_news_num, self.max_content_length, self.word_embedding_dim]) # [batch_size * news_num, max_content_length, word_embedding_dim]
        # 2. CNN encoding
        #title_c = self.dropout_(self.title_conv(title_w.permute(0, 2, 1)).permute(0, 2, 1))                              # [batch_size * news_num, max_title_length, cnn_kernel_num]#[800, 20, 400]
        content_c = self.dropout_(self.content_conv(content_w.permute(0, 2, 1)).permute(0, 2, 1))       #[80, 128, 400]    # [batch_size * news_num, max_content_length, cnn_kernel_num]
        #TRC = self.lin1(self._cnn_resnet(title_w.transpose(-2, -1)))


        '''output1, (final_hidden_state1, final_cell_state1) = self.lstm_net(title_w.transpose(0, 1))#[20, 40, 400] torch.Size([4, 40, 200])
        news_repr1 = self.attention_net_with_w(output1.permute(1, 0, 2), final_hidden_state1.permute(1, 0, 2))#[40, 200]
        output2, (final_hidden_state2, final_cell_state2) = self.lstm_net2(output1)
        news_repr2 = self.attention_net_with_w(output2.permute(1, 0, 2), final_hidden_state2.permute(1, 0, 2))'''

        output1, (final_hidden_state1, final_cell_state1) = self.lstm_net(title_w.transpose(0, 1))  # [20, 40, 400] torch.Size([4, 40, 200])
        news_repr1 = self.attention_net_with_w(output1.permute(1, 0, 2),final_hidden_state1.permute(1, 0, 2))  # [40, 200]
        output2, (final_hidden_state2, final_cell_state2) = self.lstm_net2(output1)
        news_repr2 = self.attention_net_with_w(output2.permute(1, 0, 2), final_hidden_state2.permute(1, 0, 2))
        #title_c = torch.cat((news_repr1, news_repr2), dim=-1).unsqueeze(dim=1).repeat(1, self.max_title_length, 1)
        #title_c = self.lin1(torch.cat((output1, output2), dim=-1)).view(batch_size,news_num,-1)
        title_representation = torch.cat((news_repr1, news_repr2), dim=-1).view(batch_size, news_num, -1)
        #title=self.decay_attention(output1.transpose(0,1))

        '''out1, (final_hidden1, final_cell1) = self.lstm_net(content_w.transpose(0, 1))#[128,40,400] [4, 40, 200]
        #news1 = self.attention_net_with_w(out1.permute(1, 0, 2),final_hidden1.permute(1, 0, 2))  # [40, 200]
        out2, (final_hidden2, final_cell2) = self.lstm_net2(out1)#[128,40,400]
        news=self.lin1(torch.cat((out1,out2),dim=-1))#[128,40,800]
        news2 =self.Iattention_net_with_w(news.permute(1, 0, 2), final_hidden2.permute(1, 0, 2))#[40，200]
        content_c=self.lin(news2.unsqueeze(dim=1).repeat(1, self.max_content_length,  1))#[40, 128, 400][400, 128, 400]
        content_representation=self.lin2(content_c.transpose(-1,-2)).unsqueeze(dim=-1).view([batch_size, news_num, self.cnn_kernel_num])
        #print(content_c.size())
        #content_c=torch.cat((news1,news2),dim=-1).permute(1,0,2)'''

        # 3. attention layer
        #title_representation = self.title_attention(title_c).view([batch_size, news_num, self.cnn_kernel_num])                               # [batch_size, news_num, cnn_kernel_num][16, 5, 400]
        content_representation = self.content_attention(content_c).view([batch_size, news_num, self.cnn_kernel_num])                         # [batch_size, news_num, cnn_kernel_num]
...
class CAUM(NewsEncoder):
    def __init__(self, config: Config):
        super(CAUM, self).__init__(config)
        self.max_title_length = config.max_title_length
        self.max_content_length = config.max_abstract_length
        self.cnn_kernel_num = config.cnn_kernel_num
        self.news_embedding_dim = config.cnn_kernel_num
